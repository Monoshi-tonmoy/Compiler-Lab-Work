...
%token<int_val> CHAR INT FLOAT DOUBLE IF ELSE WHILE FOR CONTINUE BREAK VOID RETURN PRINT DO DHORI RO LO
...
statement:ID LO expression // a>>>2 type statements will match here and will generate MIPS
    {
...
if_statement: IF LPAREN expression RPAREN { if($3==GT) gen_code(JUMP_GT, 1); else if($3==LT) gen_code(JUMP_LT, 1); else if($3==GTE) gen_code(JUMP_GTE, 1);  else if($3==LTE) gen_code(JUMP_LTE, 1); } LBRACE statements {gen_code(GOTO,2);} RBRACE {gen_code(LABEL,1);} optional_else;    
...
//expression type is generating from here like $$=GT
expression:
